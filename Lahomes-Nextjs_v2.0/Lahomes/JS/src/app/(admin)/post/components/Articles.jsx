...
const Articles = () => {
  return <Col xl={7} lg={12}>
      <Card>
        <CardBody>
          {articleData.map((item, idx) => <div className={`border-bottom pb-3 ${idx != 0 && 'py-3'} `} key={idx}>
              ...
              <div className="mb-2 mt-1">
                {item.tags.map((tag, idx) => <React.Fragment key={idx}>
                    <span className={`badge px-2 py-1 bg-${tag == 'Tutorials' ? 'success' : tag == 'Blog' ? 'danger' : 'primary'}-subtle text-${tag == 'Tutorials' ? 'success' : tag == 'Blog' ? 'danger' : 'primary'} ms-1`}>
                      {tag}
                    </span>
                    &nbsp;
                  </React.Fragment>)}
              ...
              <div className="d-flex align-items-center gap-1">
                ...
                <div className="ms-auto">
                  <span>
                    <button type="button" className="btn btn-soft-danger avatar-sm d-inline-flex align-items-center justify-content-center fs-20 rounded-circle">
                      <span>
                        {' '}
